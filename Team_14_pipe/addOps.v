...
// carry in for this adder will be set to 0;
    ...
    generate
        ...
        for(i=0;i<16;i=i+1) begin: myloop
            if(i == 0) begin
                addFourBitAdder fadd(.a(a[3:0]), .b(b[3:0]), .cin(1'b0), .cout(carrys[0]), .sum(res[3:0]), .msbCarryIn(msbCarryIn[i]));
            end
            else begin
                addFourBitAdder fadd(.a(a[4*i + 3:4*i]), .b(b[4*i + 3:4*i]), .cin(carrys[i-1]), .cout(carrys[i]), .sum(res[4*i + 3:4*i]), .msbCarryIn(msbCarryIn[i]));
            end
        end
    endgenerate
    xor x1(overflow_flag, msbCarryIn[15], carrys[15]);
    assign carry_flag = carrys[15];
