...
module sltOps(
    ...
    wire [63:0]atemp;
    wire [63:0]btemp;
    wire msbb_not, msba_not;
    // bias
    // adding 2^63 to a so that we can compare negative awa positive nums
    assign atemp[62:0] = a[62:0];
    not n1 (msba_not, a[63]);
    assign atemp[63] = msba_not;
    // adding 2^63 to b so that we can compare negative awa positive nums
    assign btemp[62:0] = b[62:0];
    not n2 (msbb_not, b[63]);
    assign btemp[63] = msbb_not;
    generate
        genvar i;
        for(i=0;i<16;i=i+1)begin:myloop
            if(i==0)begin:ifstat
                sltComparator cc(.a(atemp[i*4+3:i*4]), .b(btemp[i*4+3:i*4]), .l(1'b0), .eq(1'b1), .g(1'b0), .alb(temps[i][0]), .aeqb(temps[i][1]), .agb(temps[i][2]));
            end
            else begin:elsestatement
                sltComparator cc(.a(atemp[i*4+3:i*4]), .b(btemp[i*4+3:i*4]), .l(temps[i-1][0]), .eq(temps[i-1][1]), .g(temps[i-1][2]), .alb(temps[i][0]), .aeqb(temps[i][1]), .agb(temps[i][2]));
            end
        end
    endgenerate
