...
The base datapath blocks (ALU, Control Unit, Register File, Data Memory, Instruction Memory, Immediate Generator, ALU Control, Adder, ShiftLeft1, MUX) remain functionally identical to the sequential design and are not re-described here.
...
\begin{itemize}
    \item \textbf{\texttt{IFID*}} --- signals latched in the IF/ID pipeline register (e.g., \texttt{IFIDIR}, \texttt{IFIDPC}).
    \item \textbf{\texttt{IDEX*}} --- signals latched in the ID/EX register (e.g., \texttt{IDEXrs1}, \texttt{IDEXALUOp}, \texttt{IDEXrd}).
    \item \textbf{\texttt{EXMEM*}} --- signals latched in the EX/MEM register (e.g., \texttt{EXMEMALUResult}, \texttt{EXMEMrd}).
    \item \textbf{\texttt{MEMWB*}} --- signals latched in the MEM/WB register (e.g., \texttt{MEMWBReadData}, \texttt{MEMWBrd}).
...
\begin{tabular}{lcp{7cm}}
    ...
    \midrule
    \texttt{IFIDIR} & 32 & Fetched instruction. Aliased as \texttt{Instruction} via a continuous assign. \\
...
\begin{itemize}
    ...
    \item When \texttt{IFFlush = 1} (taken branch), \texttt{IFIDIR} is overwritten with the NOP instruction to squash the wrongly fetched instruction.
\end{itemize}
...
\begin{tabular}{lcp{7cm}}
    ...
    \midrule
    \texttt{IDEXALUOp} & 2 & ALU operation category \\
    \texttt{IDEXALUSrc} & 1 & ALU input B select \\
    \texttt{IDEXMemRead} & 1 & Memory read enable \\
    \texttt{IDEXMemWrite} & 1 & Memory write enable \\
    \texttt{IDEXMemtoReg} & 1 & Write-back source select \\
...
\begin{tabular}{lcp{7cm}}
    ...
    \texttt{EXMEMrs2} & 64 & Forwarded rs2 value (for store data), sourced from \texttt{ALUm2Op} \\
...
\begin{tabular}{ccl}
    \toprule
    \texttt{ForwardA/B} & \textbf{MUX input} & \textbf{Condition (A uses rs1Addr, B uses rs2Addr)} \\
    \midrule
    \texttt{2'b00} & \texttt{IDEXrs1}/\texttt{IDEXrs2} & No hazard \\
    \texttt{2'b10} & \texttt{EXMEMALUResult} & EX hazard: \texttt{EXMEMRegWrite \&\& EXMEMrd $\neq$ 0 \&\& EXMEMrd == src} \\
    \texttt{2'b01} & \texttt{write\_back} & MEM hazard: \texttt{MEMWBRegWrite \&\& MEMWBrd $\neq$ 0 \&\& MEMWBrd == src} \\
    \bottomrule
\end{tabular}
\end{center}

EX hazard has priority over MEM hazard (checked first via \texttt{if}/\texttt{else if}).
...
A \texttt{mux} instance (\texttt{sdMUX}) selects between \texttt{EXMEMrs2} (normal) and \texttt{write\_back} (forwarded) based on \texttt{Forward\_sd}. The output (\texttt{sdMUXOp}) feeds into the Data Memory's write port.
...
\subsection{Why Not ``Always Not Taken''?}
The project specification suggests resolving branches in EX with a ``predict not taken'' policy (flush 2 instructions on misprediction). Our implementation resolves in ID, which is strictly better: on a taken branch we flush only 1 instruction; on a not-taken branch we flush 0. The comparator and branch forwarding unit are the added hardware cost.
...
\end{lstlisting}
The \texttt{sd} instruction depends on \texttt{x20} which was just loaded. After the load-use stall and forwarding, \texttt{x20}'s value is correctly written to memory. Additionally, the \texttt{Forward\_sd} path handles cases where the store data comes from MEM/WB.

\subsection{Write-Before-Read Bypass (3-cycle-ago dependency)}
\begin{lstlisting}[style=verilog, numbers=none]
...
\end{lstlisting}
When \texttt{ld x2} is in WB and \texttt{add x3} is in ID, the register file's combinational write-before-read bypass ensures \texttt{x2}'s loaded value is available directly. No forwarding unit is needed for this case --- the Register File handles it internally.
...
\begin{figure}[H]
    ...
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}
        \textbf{[PLACEHOLDER: EX-to-EX and MEM-to-EX Forwarding Waveform]}\\[6pt]
        Show: \texttt{clk}, \texttt{Instruction}, \texttt{ForwardA}, \texttt{ForwardB},
        \texttt{ALUm1Op}, \texttt{ALUm2Op}, \texttt{EXMEMALUResult}, \texttt{write\_back}.
    \vspace{2cm}}}
    \caption{Data forwarding waveform.}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}
        \textbf{[PLACEHOLDER: Load-Use Stall Waveform]}\\[6pt]
        Show: \texttt{clk}, \texttt{Instruction}, \texttt{PCWrite}, \texttt{IFIDEnable},
        \texttt{Bubble}, \texttt{IDEXMemRead}, \texttt{IDEXrd}.
    \vspace{2cm}}}
    \caption{Load-use hazard stall waveform.}
    \label{fig:load-use-waveform}
\end{figure}

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}
        \textbf{[PLACEHOLDER: Branch Taken --- Stall + Flush Waveform]}\\[6pt]
        Show: \texttt{clk}, \texttt{Instruction}, \texttt{IFIDIR}, \texttt{Branch},
        \texttt{rs1EqualRs2}, \texttt{IFFlush}, \texttt{ForwardCompA}, \texttt{ForwardCompB},
        \texttt{Bubble}, \texttt{PCWrite}.
    \vspace{2cm}}}
    \caption{Branch taken with stall and flush waveform.}
    \label{fig:branch-waveform}
\end{figure}

\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{2cm}
        \textbf{[PLACEHOLDER: Store-Data Forwarding Waveform]}\\[6pt]
        Show: \texttt{clk}, \texttt{Instruction}, \texttt{Forward\_sd}, \texttt{sdMUXOp},
        \texttt{EXMEMrs2}, \texttt{write\_back}.
    \vspace{2cm}}}
    \caption{Store-data forwarding waveform.}
    \label{fig:sd-forward-waveform}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Branch resolution in ID stage} --- Reduces branch penalty from 2 to 1 cycle. Requires added hardware: \texttt{equalityComparator}, \texttt{BranchForwardingUnit}, two \texttt{mux4x2} instances (\texttt{compMuxA}, \texttt{compMuxB}).

    \item \textbf{Store-data forwarding} --- Handles the case where \texttt{sd}'s data register matches MEM/WB's destination. Uses \texttt{Forward\_sd} signal and \texttt{sdMUX}, avoiding an unnecessary stall.

    \item \textbf{Branch-specific stall logic in HDU} --- The HDU distinguishes between \texttt{beq} and non-branch instructions: for \texttt{beq}, it checks both the N-1 (ID/EX) and N-2 (EX/MEM load) dependencies, while for non-branch instructions it only checks the standard load-use hazard.
\end{enumerate}

% ══════════════════════════════════════════════════════════════════════════
%  10. NEW FILES ADDED
% ══════════════════════════════════════════════════════════════════════════
\section{New Files Added (on top of Sequential)}
...
\textbf{Modified files:} \texttt{processor.v} (rewritten for pipelining), \texttt{PC.v} (added \texttt{enable} port).\\
\textbf{Unchanged files:} \texttt{alu.v}, \texttt{ALUControl.v}, \texttt{Control.v}, \texttt{DataMem.v}, \texttt{ImmGen.v}, \texttt{instruction\_mem.v}, \texttt{mux.v}, \texttt{RegisterFile.v}, \texttt{Adder.v}, \texttt{Shiftleft1.v}, and all ALU sub-modules.
...
# Compile the system-level testbench
iverilog -o pipe_tb.vvp pipe_tb.v
